...
module Atomic
  ...
  class Server
    ...
    def self.run(host = "0.0.0.0", port = 5000)
      @server = Cool.io::TCPServer.new(host, port, Atomic::Connection)
  ...
  class Route
    ...
    class << self
      ...
      def get(path, &block);
        endpoint = Atomic::Endpoint.generate(header(path), &block)
        Atomic::Router.add endpoint
      end      
  ...
  class Connection < Cool.io::TCPSocket
    ...
    def on_read(data)
      ...
      write RocketAMF.serialize(Atomic::Router.respond(request[:header], request[:data]),3)
    end
  end
end
